...
export function SeoDiscoverySection({ value, onChange, contentTitle, contentSummary, contentBody = "", routePrefix, disabled = false, compact = false, embedded = false }: Props) {
  ...
  const checks = useMemo<Check[]>(() => [
    ...
    { code: "social_image_alt", status: value.social_media_id && !value.social_image_alt ? "WARNING" : "PASS", message: value.social_media_id && !value.social_image_alt ? "Lengkapi teks alternatif utama aset melalui Pustaka Media." : "Gambar cadangan atau gambar sosial memiliki teks alternatif." },
  ...
  return <section className="admin-form-card" aria-labelledby={`${id}-title`}>
    ...
    <div className="admin-form-body">
      ...
      <div className="grid gap-6 md:grid-cols-2"><div><label htmlFor={`${id}-seo-title`} className="admin-label">Judul SEO</label><input id={`${id}-seo-title`} className="admin-input" value={value.seo_title} disabled={disabled} maxLength={200} onChange={(event) => set("seo_title", event.target.value)} placeholder={contentTitle || "Mengikuti judul konten"} /></div><div><label htmlFor={`${id}-description`} className="admin-label">Deskripsi meta</label><textarea id={`${id}-description`} className="admin-input" rows={3} value={value.meta_description} disabled={disabled} maxLength={500} onChange={(event) => set("meta_description", event.target.value)} placeholder={contentSummary || "Mengikuti ringkasan konten"} /></div></div>
      <div className="grid gap-5 xl:grid-cols-2"><section className="rounded-2xl border border-slate-200 bg-white p-5" aria-label="Search preview"><p className="admin-kicker">Search preview</p><p className="mt-3 text-lg font-bold text-sky-800">{effectiveTitle}</p><p className="mt-1 truncate text-xs text-emerald-700">teman-belajar.local{canonical}</p><p className="mt-2 text-sm leading-6 text-slate-600">{effectiveDescription}</p></section><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white" aria-label="Pratinjau sosial"><div className="flex aspect-[2/1] items-center justify-center bg-sky-50 text-sm font-bold text-sky-700">{value.social_media_id ? selectedMediaName || "Media sosial terpilih" : "Gambar sosial belum dipilih"}</div><div className="p-5"><p className="font-black text-slate-900">{socialTitle}</p><p className="mt-2 text-sm text-slate-600">{socialDescription}</p></div></section></div>
      ...
      <details className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><summary className="cursor-pointer font-black text-slate-900">SEO lanjutan</summary><div className="mt-5 grid gap-5"><div className="grid gap-5 md:grid-cols-2"><div><label htmlFor={`${id}-social-title`} className="admin-label">Judul sosial</label><input id={`${id}-social-title`} className="admin-input" disabled={disabled} value={value.social_title} maxLength={200} onChange={(event) => set("social_title", event.target.value)} /></div><div><label htmlFor={`${id}-social-description`} className="admin-label">Deskripsi sosial</label><textarea id={`${id}-social-description`} className="admin-input" disabled={disabled} rows={2} value={value.social_description} maxLength={500} onChange={(event) => set("social_description", event.target.value)} /></div></div><div><label htmlFor={`${id}-canonical`} className="admin-label">Path URL utama internal (opsional)</label><input id={`${id}-canonical`} className="admin-input" disabled={disabled} value={value.canonical_path || ""} onChange={(event) => set("canonical_path", event.target.value || null)} placeholder={`${routePrefix}${value.slug || "slug-konten"}`} /><p className="mt-2 text-xs text-slate-500">Hanya path internal sesuai tipe konten; URL eksternal dan kueri ditolak server.</p></div><label className="flex items-start gap-3 text-sm font-bold text-slate-700"><input type="checkbox" className="mt-1" checked={value.indexable === "true"} disabled={disabled} onChange={(event) => set("indexable", String(event.target.checked))} /><span>Izinkan pengindeksan setelah konten berstatus terbit dan memenuhi kebijakan publik.</span></label></div></details>
      <section aria-labelledby={`${id}-health`}><h3 id={`${id}-health`} className="font-black text-slate-900">SEO Health Assistant</h3><p className="mt-1 text-xs text-slate-500">Daftar periksa editorial, bukan skor atau janji peringkat mesin pencari.</p><ul className="mt-4 grid gap-3 sm:grid-cols-2">{checks.map((check) => <li key={check.code} className="flex gap-3 rounded-xl border border-slate-200 bg-white p-4"><span className={`h-fit rounded-full px-2 py-1 text-[10px] font-black ${check.status === "PASS" ? "bg-emerald-50 text-emerald-700" : check.status === "BLOCKER" ? "bg-rose-50 text-rose-700" : "bg-sky-50 text-sky-800"}`}>{check.status}</span><span className="text-xs leading-5 text-slate-600">{check.message}</span></li>)}</ul></section>
    </div>
  </section>;
}
