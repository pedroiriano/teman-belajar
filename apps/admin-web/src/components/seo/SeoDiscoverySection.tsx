...
export function SeoDiscoverySection({ value, onChange, contentTitle, contentSummary, contentBody = "", routePrefix, disabled = false, compact = false, embedded = false }: Props) {
  ...
  const checks = useMemo<Check[]>(() => [
    ...
    { code: "social_image_alt", status: value.social_media_id && !value.social_image_alt ? "WARNING" : "PASS", message: value.social_media_id && !value.social_image_alt ? "Lengkapi teks alternatif kanonis aset melalui Pustaka Media." : "Gambar cadangan atau gambar sosial memiliki teks alternatif." },
  ...
  return <section className="admin-form-card" aria-labelledby={`${id}-title`}>
    ...
    <div className="admin-form-body">
      ...
      <div className="grid gap-5 xl:grid-cols-2"><section className="rounded-2xl border border-slate-200 bg-white p-5" aria-label="Pratinjau pencarian"><p className="admin-kicker">Pratinjau pencarian</p><p className="mt-3 text-lg font-bold text-sky-800">{effectiveTitle}</p><p className="mt-1 truncate text-xs text-emerald-700">teman-belajar.local{canonical}</p><p className="mt-2 text-sm leading-6 text-slate-600">{effectiveDescription}</p></section><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white" aria-label="Pratinjau sosial"><div className="flex aspect-[2/1] items-center justify-center bg-sky-50 text-sm font-bold text-sky-700">{value.social_media_id ? selectedMediaName || "Media sosial terpilih" : "Gambar sosial belum dipilih"}</div><div className="p-5"><p className="font-black text-slate-900">{socialTitle}</p><p className="mt-2 text-sm text-slate-600">{socialDescription}</p></div></section></div>
