            ...
            <div className="ml-auto flex items-center gap-2 sm:gap-3">
              ...
              <details className="relative">
                <summary className="flex cursor-pointer list-none items-center gap-2 rounded-xl p-1.5 transition hover:bg-slate-100">
                  <span className="grid h-10 w-10 place-items-center rounded-xl bg-orange-100 text-sm font-black text-orange-700">{initials}</span>
                  <span className="hidden text-left xl:block"><span className="block max-w-40 truncate text-sm font-bold text-slate-800">{userName || userEmail}</span><span className="block text-xs text-slate-500">{role}</span></span>
                  <AdminIcon name="chevron" className="hidden h-4 w-4 text-slate-400 xl:block" />
                </summary>
                <div className="admin-profile-menu"><div className="border-b p-4"><p className="text-sm font-bold text-slate-900">{userName || "Pengguna Teman Belajar"}</p><p className="mt-1 truncate text-xs text-slate-500">{userEmail}</p></div><Link href="/api/auth/federated-logout" prefetch={false}>Keluar dari Admin</Link></div>
              </details>
            </div>
          </div>
