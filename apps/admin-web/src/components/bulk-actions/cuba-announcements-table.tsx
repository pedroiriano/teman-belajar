...
interface CubaAnnouncementsTableProps {
...
export function CubaAnnouncementsTable({
  announcements,
  itemCount,
  headerActions,
  errorMessage,
}: CubaAnnouncementsTableProps) {
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [sortKey, setSortKey] = useState<string>("schedule");
  ...
  }, [announcements, sortKey, sortDirection]);

  const isAllSelected = announcements.length > 0 && selectedIds.size === announcements.length;
  const isSomeSelected = selectedIds.size > 0 && selectedIds.size < announcements.length;
  ...
  const selectedItems: BulkSelectedItem[] = announcements
  ...
  const handleConfirmBulk = async () => {
    ...
    try {
      ...
      const res = await executeBulkActionAction("announcements", activeAction, selectedItems);
    ...
    <>
      <AdminDataTable
        ...
        headers={[
          ...
          { label: "Jadwal tayang", key: "schedule", sortable: true },
      ...
      >
        {sortedAnnouncements.map((ann) => {
          const isChecked = selectedIds.has(ann.id);
          return (
            <tr
              key={ann.id}
            ...
            >
              <td className="w-10 px-4 py-3 text-center">
                <input
                  ...
                  checked={isChecked}
                  onChange={() => handleToggleRow(ann.id)}
              ...
              <td className="p-4" data-label="Status">
                ...
                >
                  {statusLabels[ann.status] || ann.status.replace("_", " ")}
                </span>
              </td>
              <td
                className="p-4 text-xs text-slate-600 dark:text-slate-400"
                data-label="Jadwal tayang"
              >
                <div>
                  <span className="font-medium text-slate-700 dark:text-slate-300">Mulai:</span>{" "}
                  {ann.start_at ? new Date(ann.start_at).toLocaleDateString("id-ID") : "-"}
                </div>
                <div className="mt-0.5">
                  <span className="font-medium text-slate-700 dark:text-slate-300">Selesai:</span>{" "}
                  {ann.end_at ? new Date(ann.end_at).toLocaleDateString("id-ID") : "-"}
                </div>
              </td>
