...
export function AdminPagination({ page, pages, total, pageSize, pathname, query = {}, pageSizeOptions = [10, 20, 50] }: AdminPaginationProps) {
  ...
  return <nav className="cuba-pagination admin-pagination" aria-label="Paginasi data">
    ...
    <div className="admin-page-size"><span>Data per halaman</span>{pageSizeOptions.map((size) => <Link key={size} href={href(1, size)} scroll={false} aria-current={size === pageSize ? "true" : undefined} className={size === pageSize ? "is-active" : ""}>{size}</Link>)}</div>
    <div className="admin-pagination-controls">{current > 1 ? <Link className="admin-button-secondary !min-h-9 !px-3" href={href(current - 1)} scroll={false} rel="prev">Sebelumnya</Link> : <span className="admin-button-secondary !min-h-9 !px-3 opacity-50" aria-disabled="true">Sebelumnya</span>}<div className="admin-page-numbers">{numbers(current, pages).map((value, index) => value === "…" ? <span key={`ellipsis-${index}`} aria-hidden="true">…</span> : <Link key={value} href={href(value)} scroll={false} aria-label={`Halaman ${value}`} aria-current={value === current ? "page" : undefined} className={value === current ? "is-active" : ""}>{value}</Link>)}</div>{current < pages ? <Link className="admin-button-secondary !min-h-9 !px-3" href={href(current + 1)} scroll={false} rel="next">Berikutnya</Link> : <span className="admin-button-secondary !min-h-9 !px-3 opacity-50" aria-disabled="true">Berikutnya</span>}</div>
  </nav>;
}
