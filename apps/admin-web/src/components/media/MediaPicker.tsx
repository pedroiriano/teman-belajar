...
export default function MediaPicker({ onSelect, buttonLabel = "Pilih media" }: Props) {
  ...
  return <>
    <button ref={triggerRef} type="button" onClick={() => { setIsOpen(true); setSelected(null); setError(""); }} className="admin-button-secondary"><AdminIcon name="media" className="mr-2 h-4 w-4" />{buttonLabel}</button>
    {isOpen && <div className="admin-modal-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) setIsOpen(false); }}>
      <section ref={modalRef} className="admin-modal" role="dialog" aria-modal="true" aria-labelledby={titleId}>
        ...
        <div className="border-b px-5 pt-3 sm:px-6" role="tablist" aria-label="Sumber media"><button type="button" role="tab" aria-selected={tab === "library"} className={`border-b-2 px-4 py-3 text-sm font-black ${tab === "library" ? "border-sky-500 text-sky-700" : "border-transparent text-slate-500"}`} onClick={() => setTab("library")}>Media Library</button><button type="button" role="tab" aria-selected={tab === "upload"} className={`border-b-2 px-4 py-3 text-sm font-black ${tab === "upload" ? "border-sky-500 text-sky-700" : "border-transparent text-slate-500"}`} onClick={() => setTab("upload")}>Unggah Baru</button></div>
