...
export function validateMoodleInitiatedAdminBridgeRequest(requestUrl: URL): URL | null {
  const secret = getBridgeSecret();
  const keycloakLogout = getVerifiedReturnUrl(requestUrl, secret);
  return keycloakLogout && isExpectedMoodleKeycloakLogoutUrl(keycloakLogout) ? keycloakLogout : null;
}
