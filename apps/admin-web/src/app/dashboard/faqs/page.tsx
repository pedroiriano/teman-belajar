    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(320px,.72fr)_minmax(0,1.28fr)]">
      <section className="admin-form-card h-fit" aria-labelledby="faq-list-title">
        ...
        <div className="admin-form-body !gap-4">
          ...
          {loading ? <div className="admin-empty" role="status">Memuat FAQ…</div> : items.length === 0 ? <div className="admin-empty rounded-xl border border-slate-200">Belum ada FAQ yang sesuai.</div> : <ul className="grid gap-3">{items.map((item)=><li key={item.id}><button type="button" onClick={()=>choose(item)} className="w-full rounded-xl border border-slate-200 p-4 text-left transition hover:border-sky-400" data-selected={selected?.id===item.id}><div className="flex items-start justify-between gap-3"><span className="font-black text-slate-900">{item.question}</span><span className="admin-status admin-status-neutral shrink-0">{({ draft: 'Draf', in_review: 'Peninjauan', published: 'Terbit', archived: 'Diarsipkan', approved: 'Disetujui', rejected: 'Ditolak' } as Record<string, string>)[item.status] || item.status.replace('_', ' ')}</span></div><p className="mt-2 text-xs text-slate-500">{item.category_name} · /{item.slug}</p></button></li>)}</ul>}
