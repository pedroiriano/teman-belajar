...
export default function FAQWorkspacePage() {
  ...
  const load = useCallback(async (selectId?: string) => {
    setLoading(true);
    const result = await getFAQWorkspaceAction({ q: appliedQuery, status, categoryId: category, page, pageSize });
    if (!result.success) { setError(result.error); setLoading(false); return; }
    if (result.pagination.total_pages > 0 && result.pagination.page > result.pagination.total_pages) { setPage(result.pagination.total_pages); setLoading(false); return; }
    setCategories(result.categories); setItems(result.items); setRoles(result.roles); setPagination(result.pagination); setError(""); setLoading(false);
  ...
  useEffect(() => {
    let active = true;
    void getFAQWorkspaceAction({ q: appliedQuery, status, categoryId: category, page, pageSize }).then((result) => {
      if (!active) return;
      if (!result.success) { setError(result.error); setLoading(false); return; }
      if (result.pagination.total_pages > 0 && result.pagination.page > result.pagination.total_pages) { setPage(result.pagination.total_pages); setLoading(false); return; }
      setCategories(result.categories); setItems(result.items); setPagination(result.pagination); setRoles(result.roles); setLoading(false);
    });
    return () => { active = false; };
  }, [appliedQuery, category, page, pageSize, status]);
  ...
  return <div className="admin-page">
    <header className="admin-page-header">
      <div className="max-w-3xl"><p className="admin-kicker">Pusat Bantuan</p><h1 className="admin-page-title">FAQ</h1><p className="admin-page-copy">Kelola jawaban singkat dengan alur kerja editorial yang sama ketatnya seperti konten lain.</p></div>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(320px,.72fr)_minmax(0,1.28fr)]">
      <section className="admin-form-card h-fit" aria-labelledby="faq-list-title">
        <div className="admin-form-header"><div><h2 id="faq-list-title" className="font-black text-slate-900">Daftar FAQ</h2><p className="mt-1 text-xs text-slate-500">{pagination.total} data di seluruh alur kerja</p></div></div>
        <div className="admin-form-body !gap-4">
          <form onSubmit={(event)=>{event.preventDefault();setPage(1);setAppliedQuery(query.trim());}} className="flex gap-2"><div className="grow"><label htmlFor="faq-search" className="sr-only">Cari FAQ</label><input id="faq-search" type="search" className="admin-input" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Cari pertanyaan atau jawaban…" /></div><button className="admin-button-secondary" type="submit">Cari</button></form>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><div><label className="sr-only" htmlFor="faq-status">Status</label><select id="faq-status" className="admin-input" value={status} onChange={(event)=>{setStatus(event.target.value as StatusFilter);setPage(1);}}><option value="all">Semua status</option><option value="draft">Draf</option><option value="in_review">Dalam peninjauan</option><option value="approved">Disetujui</option><option value="published">Terbit</option><option value="archived">Arsip</option></select></div><div><label className="sr-only" htmlFor="faq-category-filter">Kategori</label><select id="faq-category-filter" className="admin-input" value={category} onChange={(event)=>{setCategory(event.target.value);setPage(1);}}><option value="">Semua kategori</option>{categories.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div></div>
          {loading ? <div className="admin-empty" role="status">Memuat FAQ…</div> : items.length === 0 ? <div className="admin-empty rounded-xl border border-slate-200 dark:border-slate-800">Belum ada FAQ yang sesuai.</div> : <ul className="grid gap-3">{items.map((item)=><li key={item.id}><button type="button" onClick={()=>choose(item)} className="w-full rounded-xl border border-slate-200 dark:border-slate-800 p-4 text-left transition hover:border-sky-400" data-selected={selected?.id===item.id}><div className="flex items-start justify-between gap-3"><span className="font-bold text-slate-900 dark:text-slate-100">{item.question}</span><span className={`cuba-badge shrink-0 text-[10px] ${faqStatusBadgeClasses[item.status] || "cuba-badge-neutral"}`}>{({ draft: 'Draf', in_review: 'Peninjauan', published: 'Terbit', archived: 'Diarsipkan', approved: 'Disetujui' } as Record<string, string>)[item.status] || item.status.replace('_', ' ')}</span></div><p className="mt-2 text-xs text-slate-500 dark:text-slate-400">{item.category_name} · /{item.slug}</p></button></li>)}</ul>}
          <AdminClientPagination page={pagination.page} pages={pagination.total_pages} total={pagination.total} pageSize={pagination.page_size} onPageChange={setPage} onPageSizeChange={(size)=>{setPageSize(size);setPage(1);}} />
        </div>
      </section>
      <section>{creating || selected ? <FAQEditor key={selected?.id || "create"} item={selected} categories={activeCategories} canWrite={canWrite} roles={roles} onSaved={async (id,message)=>{setNotice(message);setCreating(false);await load(id);}} onChanged={async (message)=>{setNotice(message);await load(selected?.id);}} onError={setError} onCancel={()=>{setCreating(false);setSelected(null);}} /> : <div className="admin-empty rounded-2xl border border-slate-200 bg-white"><span className="admin-stat-icon mx-auto"><AdminIcon name="folder" className="h-5 w-5"/></span><h2 className="mt-4 text-xl font-black text-slate-900">Pilih FAQ untuk melihat detail</h2><p className="mt-2">Atau buat FAQ baru untuk memulai alur kerja editorial.</p></div>}</section>
    </div>
  </div>;
}
