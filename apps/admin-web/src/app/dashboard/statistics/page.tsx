  ...
  return (
    <div className="admin-page space-y-10">
      ...
      <Section id="api" title="API" description="Sinyal Prometheus; nol, no-series, invalid, dan unavailable memiliki state berbeda.">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="Tingkat Permintaan" value={`${renderPromValue(data.api.request_rate)} /s`} note="Rata-rata 5 menit." /><StatCard label="Ketersediaan" value={renderPromValue(data.api.availability, "percent")} note="up API selama 5 menit." /><StatCard label="Tingkat Kesalahan" value={renderPromValue(data.api.error_rate, "percent")} note="Proporsi respons 5xx." /><StatCard label="Latensi p95" value={renderPromValue(data.api.p95_latency, "ms")} note="Persentil ke-95." /><StatCard label="Latensi p99" value={renderPromValue(data.api.p99_latency, "ms")} note="Persentil ke-99." /></div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="2xx Sukses" value={renderPromValue(data.api.status_2xx)} note="Respons sukses per detik." /><StatCard label="3xx Redireksi" value={renderPromValue(data.api.status_3xx)} note="Redirect per detik." /><StatCard label="4xx Kesalahan Klien" value={renderPromValue(data.api.status_4xx)} note="Client error per detik." /><StatCard label="5xx Kesalahan Server" value={renderPromValue(data.api.status_5xx)} note="Server error per detik." /><StatCard label="Latensi p50" value={renderPromValue(data.api.p50_latency, "ms")} note="Median latency." /></div>
      </Section>
      ...
      <Section id="search" title="Pencarian & Engagement" description="Search tanpa raw query; engagement diberi label current/all-time state.">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Total Pencarian" value={number(totalSearches)} note="search.executed dalam periode." /><StatCard label="Hasil Kosong" value={number(zeroResults)} note="search.zero_result dalam periode." /><StatCard label="Sukses Pencarian" value={percent(searchSuccessRate)} note="(Search - zero result) / search." /><StatCard label="Rasio Klik Hasil" value={percent(searchClickRate)} note="Result clicks / total searches." /></div>
        <DataTable headers={["Tanggal", "Pencarian", "Hasil Kosong", "Klik Hasil"]} empty={!data.search?.length}>{data.search?.map((row: SearchDaily) => <tr key={row.date}><td className="px-5 py-4">{row.date}</td><td className="px-5 py-4">{number(row.total_searches)}</td><td className="px-5 py-4">{number(row.zero_results)}</td><td className="px-5 py-4">{number(row.result_clicks)}</td></tr>)}</DataTable>
        <div className="grid gap-4 sm:grid-cols-3"><StatCard label="Markah" value={data.engagement ? number(data.engagement.bookmarks) : "Tidak tersedia"} note="Current/all-time state, bukan rentang tanggal." /><StatCard label="Peringkat" value={data.engagement ? number(data.engagement.ratings) : "Tidak tersedia"} note="Current/all-time state, bukan rentang tanggal." /><StatCard label="Rata-rata Peringkat" value={data.engagement ? data.engagement.avg_rating.toLocaleString("id-ID", { maximumFractionDigits: 2 }) : "Tidak tersedia"} note="Rata-rata rating aktif saat ini." /></div>
      </Section>
