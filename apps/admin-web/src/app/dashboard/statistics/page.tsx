...
function SourceBadge({ label, source }: { label: string; source: SourceState }) {
  const badgeClass = source.status === "fresh"
    ? "cuba-badge-success"
    : source.status === "stale"
      ? "cuba-badge-warning"
      : "cuba-badge-danger";
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 px-3 py-2 dark:border-slate-700">
      ...
      <span className={`cuba-badge ${badgeClass}`}>{source.status}</span>
    </div>
  );
}
...
export default async function StatisticsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const nav = [
    ["ringkasan", "Ringkasan"],
    ["tren", "Grafik Tren & Heatmap"],
    ["engagement", "Engagement Publik"],
    ["traffic", "Pengunjung & Halaman"],
    ["content", "Konten"],
  ...
  return (
    <div className="admin-page space-y-10">
      {/* Official Executive Print Header (Only visible on @media print) */}
      <div className="cuba-print-header hidden border-b border-slate-300 pb-4 print:block">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-sky-700">
          SISTEM MANAJEMEN PEMBELAJARAN TEMAN BELAJAR
        </p>
        <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-900">
          Laporan Eksekutif Statistik & Observabilitas Platform
        </h1>
        <div className="mt-2 flex items-center justify-between text-xs text-slate-600">
          <span>Periode Analisis: {days} Hari Terakhir</span>
          <span>Dicetak Pada: {new Date().toLocaleDateString("id-ID", { dateStyle: "full" })}</span>
        </div>
      </div>
      ...
      <header className="admin-page-header print:hidden">
        <div>
          <p className="admin-kicker">ANALITIK & OBSERVABILITAS</p>
          <h1 className="admin-page-title">Statistik Platform</h1>
          <p className="admin-page-copy">
            Statistik produk yang terkurasi; dasbor observabilitas modular dengan visualisasi interaktif dan ekspor multi-format.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <Suspense fallback={<div className="h-10 w-44 animate-pulse rounded-xl bg-slate-200 dark:bg-slate-800" />}>
            <StatisticsFilter />
          </Suspense>
          <CubaStatisticsExport data={data} days={days} />
        </div>
      </header>
      ...
      <Section id="ringkasan" title="Ringkasan" description={`Snapshot terpilih untuk ${days} hari terakhir.`}>
        <div className="grid gap-3 lg:grid-cols-3">
          <SourceBadge label="Analytics Worker" source={data.sources.analytics} />
          <SourceBadge label="Moodle" source={data.sources.moodle} />
          <SourceBadge label="Prometheus" source={data.sources.prometheus} />
        </div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Tayangan Halaman Portal" value={number(totalPageViews)} note="Hanya portal.page_view; konten dan Admin tidak digandakan." />
          <StatCard label="Pengunjung Unik" value={visitors} note="Distinct visitor Portal; presisi dibatasi retensi 30 hari." />
          <StatCard label="Pembelajar Aktif" value={periodLearner ? number(periodLearner.active_learners) : "Tidak tersedia"} note="Distinct genuine learner dari Moodle untuk seluruh periode." />
          <StatCard label="Tingkat Penyelesaian" value={periodLearner ? percent(periodLearner.completion_rate) : "Tidak tersedia"} note="Completed eligible enrolments dibagi cohort eligible yang sama." />
        </div>
      </Section>

      {/* 2. Visualisasi Grafik Tren & Heatmap Kontributor (Fase 2.1) */}
      <Section id="tren" title="Grafik Tren & Heatmap Aktivitas" description="Analisis deret waktu interaktif volume modul dan matriks kepadatan aktivitas kontributor.">
      ...
      {/* 3. Metrik Engagement Konten Publik (Fase 2.1) */}
      <Section id="engagement" title="Engagement Konten Publik" description="Ulasan kepuasan pengguna, markah tersimpan, efektivitas temuan pencarian, dan peringkat konten teratas.">
      ...
      <Section id="learning" title="Pembelajaran" description="Cohort formal Moodle; staf, guest, site admin, akun nonaktif, dan pengguna tanpa role learner tidak dihitung.">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Pembelajar Aktif" value={periodLearner ? number(periodLearner.active_learners) : "Tidak tersedia"} note="Distinct learner dalam rentang terpilih." />
          <StatCard label="Mulai Belajar" value={periodLearner ? number(periodLearner.learning_starts) : "Tidak tersedia"} note="Aktivitas mulai pada cohort terpilih." />
      ...
      <Section id="api" title="API" description="Sinyal Prometheus; nol, no-series, invalid, dan unavailable memiliki state berbeda.">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          <StatCard label="Tingkat Permintaan" value={`${renderPromValue(data.api.request_rate)} /s`} note="Rata-rata 5 menit." />
          <StatCard label="Ketersediaan" value={renderPromValue(data.api.availability, "percent")} note="up API selama 5 menit." />
          <StatCard label="Tingkat Kesalahan" value={renderPromValue(data.api.error_rate, "percent")} note="Proporsi respons 5xx." />
          <StatCard label="Latensi p95" value={renderPromValue(data.api.p95_latency, "ms")} note="Persentil ke-95." />
          <StatCard label="Latensi p99" value={renderPromValue(data.api.p99_latency, "ms")} note="Persentil ke-99." />
        </div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          <StatCard label="2xx Sukses" value={renderPromValue(data.api.status_2xx)} note="Respons sukses per detik." />
          <StatCard label="3xx Redireksi" value={renderPromValue(data.api.status_3xx)} note="Redirect per detik." />
          <StatCard label="4xx Kesalahan Klien" value={renderPromValue(data.api.status_4xx)} note="Client error per detik." />
          <StatCard label="5xx Kesalahan Server" value={renderPromValue(data.api.status_5xx)} note="Server error per detik." />
          <StatCard label="Latensi p50" value={renderPromValue(data.api.p50_latency, "ms")} note="Median latency." />
        </div>
      </Section>

      {/* 8. Rincian Harian Pencarian */}
      <Section id="search" title="Pencarian & Temuan Harian" description="Log pencarian harian tanpa raw query untuk menjaga privasi pengguna.">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Total Pencarian" value={number(totalSearches)} note="search.executed dalam periode." />
          <StatCard label="Hasil Kosong" value={number(zeroResults)} note="search.zero_result dalam periode." />
