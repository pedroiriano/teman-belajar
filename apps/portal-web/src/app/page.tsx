...
export default async function Home() {
  const [faqResult, configuration] = await Promise.all([getPublicFAQs(), getPublicPlatformConfiguration()]);
  const faqs = faqResult.data.flatMap((group) => group.items).slice(0, 4);
  const sectionProps = (key: string) => { const section = configuration.homepage.sections.find((item) => item.key === key); return { hidden: section ? !section.visible : true, style: { order: section?.order ?? 99 } }; };
  ...
  return (
    <div className="flex flex-col">
      {configuration.banner.enabled ? <aside className="border-b border-sky-200 bg-sky-50" aria-label="Informasi utama" style={{ order: 0 }}><div className="portal-container flex flex-col gap-4 py-4 sm:flex-row sm:items-center">{bannerMedia ? <img src={bannerMedia} alt="" className="h-14 w-20 rounded-lg object-cover" /> : null}<div className="min-w-0 flex-1"><p className="font-black text-slate-900">{configuration.banner.title}</p><p className="mt-1 text-sm text-slate-600">{configuration.banner.body}</p></div>{configuration.banner.href ? <Link href={configuration.banner.href} className="portal-button-secondary shrink-0">Selengkapnya</Link> : null}</div></aside> : null}
      <section {...sectionProps("hero")} className="portal-course-hero" data-techwind-pattern="index-course-hero">
        ...
        <div className="portal-container relative grid min-h-[650px] items-center gap-14 py-16 lg:grid-cols-[1.05fr_.95fr] lg:py-24">
          <div>
            {configuredLogo ? <img src={configuredLogo} alt="Logo Teman Belajar" className="mb-6 h-20 w-20 object-contain drop-shadow-xl sm:h-24 sm:w-24" /> : <BrandLogo variant="main" decorative={false} priority className="mb-6 h-20 w-20 object-contain drop-shadow-xl sm:h-24 sm:w-24" />}
      ...
      <section {...sectionProps("trust")} className="border-b border-slate-100 bg-white py-6">
      ...
      <section {...sectionProps("learning_paths")} className="portal-section">
      ...
      <section {...sectionProps("topics")} className="portal-section portal-section-muted">
        <div className="portal-container">
      ...
      <section {...sectionProps("knowledge")} className="portal-section">
        <div className="portal-container">
      ...
      <section {...sectionProps("media")} id="media" className="portal-section overflow-hidden bg-[#102a43] text-white">
        <div className="portal-container grid items-center gap-12 lg:grid-cols-[.85fr_1.15fr]">
          <div><p className="text-xs font-black uppercase tracking-[.2em] text-teal-300">Media pembelajaran</p><h2 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">Visual yang hidup di dalam konten tepercaya.</h2><p className="mt-5 text-base leading-8 text-slate-300">Foto dan video dikelola melalui Pustaka Media, lalu hanya tersedia ketika koleksinya telah ditinjau dan diterbitkan.</p><Link href="/media-gallery" className="portal-button-primary mt-8 !bg-teal-500 !text-slate-950">Buka galeri media</Link></div>
      ...
      <section {...sectionProps("stats")} className="portal-section portal-section-muted">
      ...
      <section {...sectionProps("faq")} id="faq" className="portal-section">
      ...
      <section {...sectionProps("cta")} className="pb-16 sm:pb-20 lg:pb-24">
