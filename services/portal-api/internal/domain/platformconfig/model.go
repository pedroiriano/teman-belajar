...
func ApplyCurrentDefaults(config Config) Config {
	...
	if !hasNewsFeature {
		config.Features = append(config.Features, FeaturePresentation{Key: "news", Label: "Warta & Berita", Visible: true})
	}
	hasAnnouncementFeature := false
	for _, feature := range config.Features {
		if feature.Key == "announcements" {
			hasAnnouncementFeature = true
			break
		}
	}
	if !hasAnnouncementFeature {
		config.Features = append(config.Features, FeaturePresentation{Key: "announcements", Label: "Pengumuman", Visible: true})
	}
	...
	if !hasSearchFeature {
		config.Features = append(config.Features, FeaturePresentation{Key: "search", Label: "Pencarian Terpadu", Visible: true})
	}
	return config
}
