...
KEYCLOAK_PORT=$(echo "$MOODLE_KEYCLOAK_ISSUER" | sed -nE 's|https?://[^:]+:([0-9]+).*|\1|p')
...
# keycloak.teman-belajar.localhost:$KEYCLOAK_PORT (which resolves to 127.0.0.1
# inside the container due to the .localhost TLD) are forwarded to the
# actual Keycloak Docker service on its internal port.
echo "Starting Keycloak loopback proxy (socat) on port $KEYCLOAK_PORT..."
socat TCP-LISTEN:$KEYCLOAK_PORT,fork,bind=127.0.0.1 TCP:keycloak:8080 &
if [ "$KEYCLOAK_PORT" != "8081" ]; then
